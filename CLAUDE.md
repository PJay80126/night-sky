...
sw.js                   # Service worker — cache-first, version tagged (e.g. night-sky-v41)
...
## Service Worker
Cache version is hardcoded in `sw.js` as `CACHE = 'night-sky-v41'`. **Bump the version number whenever assets change** to force cache invalidation for existing installs. The assets list at the top of `sw.js` must include any new files added to the project.
